...
def collect_process():
    ...
    for feed in feed_list:
        ...
        for item in feed_output.entries:
            link = item['link']
            if database.db_check_record(link) == None: # Check if record already exists 
                ...
                if len(title) > 1:
                    ...
                    # Handle the case where there's no colon
                    title = title[0].replace('"','')  # Or implement other logic as needed
                print(f'TITLE --- {title}')
